...
int main(int argc, char *argv[]) {
    try {
        ...
        // Build model with GPU layers
        std::vector<std::unique_ptr<ILayer>> layers;
        layers.push_back(std::make_unique<ConvolutionLayerGPU>(32, 5, 5, 1, 2, 1));
        layers.push_back(std::make_unique<ReLULayer>());
        layers.push_back(std::make_unique<MaxPoolingLayer>(2, 2));
        layers.push_back(std::make_unique<ConvolutionLayerGPU>(64, 5, 5, 1, 2, 32));
        layers.push_back(std::make_unique<ReLULayer>());
        layers.push_back(std::make_unique<MaxPoolingLayer>(2, 2));
        layers.push_back(std::make_unique<FlattenLayer>());
        layers.push_back(std::make_unique<FullyConnectedLayerGPU>(3136, 128));
        layers.push_back(std::make_unique<ReLULayer>());
        layers.push_back(std::make_unique<FullyConnectedLayerGPU>(128, 10));
        ...
        std::cout << "\n=== Layer-wise Timing (GPU) ===" << std::endl;
